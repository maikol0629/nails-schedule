...
function HeroSection({ 
  ...
  const heroBodyText = stylistInfo?.bio?.trim()
    ? `${stylistInfo.bio.slice(0, BIO_LIMIT)}${
        stylistInfo.bio.length > BIO_LIMIT ? '…' : ''
      }`
  ...
  return (
    <section className="py-10 sm:py-14 grid md:grid-cols-2 gap-10 items-center">
      <div className="order-2 md:order-1 space-y-6">
        ...
        {stylistLocation && (
          <span
            className="block text-transparent bg-clip-text text-2xl sm:text-3xl lg:text-4xl font-bold mt-2"
        ...
        <div className="flex flex-wrap items-center gap-4">
          <button
            type="button"
            onClick={() => onOpenReservation(null)}
...
function BusinessAbout({ stylistInfo, theme }) {
  if (!stylistInfo?.bio?.trim()) return null;

  return (
    <section className="py-8 sm:py-10">
      <div className="max-w-3xl mx-auto">
        <div
          className="bg-white/90 p-6 sm:p-8"
          style={{
        ...
        >
          <h2 className="text-xl sm:text-2xl font-bold text-gray-900 mb-4">
            Sobre {stylistInfo.businessName || stylistInfo.ownerName || 'Nosotros'}
          </h2>
...
function ServiceCard({ service, theme, primaryColor, onOpenReservation }) {
  return (
    <article
    ...
    >
      ...
      >
        <div
          ...
          style={{
            background: `radial-gradient(circle at 0 0, ${hexToRgba(primaryColor, 0.4)} 0%, transparent 60%), radial-gradient(circle at 100% 100%, ${hexToRgba(theme.colors.accent, 0.4)} 0%, transparent 55%)`,
          }}
        />
      </div>
      ...
      <div className="p-4 pt-0">
        <button
          type="button"
          onClick={() => onOpenReservation(service.id)}
    ...
    <div className="fixed inset-0 z-40 flex items-center justify-center bg-black/40 p-4">
      <div className="bg-white rounded-2xl shadow-xl max-w-md w-full max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={onSubmit} className="p-4 space-y-4">
          ...
          {confirmation && (
            <div
              className="rounded-lg px-3 py-2 text-xs sm:text-sm flex items-start gap-2"
              style={{
          ...
          <button
            type="submit"
            disabled={submitting}
            className={`w-full inline-flex items-center justify-center gap-2 rounded-full px-5 py-2.5 text-sm sm:text-base font-semibold text-white shadow-md transition-all ${
  ...
  return (
    ...
    >
      ...
      <main className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 pb-16">
        ...
        {/* Business About */}
        <BusinessAbout stylistInfo={stylistInfo} theme={theme} />
            ...
            <div className="columns-2 md:columns-3 gap-3 sm:gap-4 space-y-3 sm:space-y-4">
              {portfolio.map((item) => (
                <button
                  type="button"
                  key={item.id}
                  className="relative w-full overflow-hidden rounded-2xl group transition-all"
                  style={{
                    outline: 'none',
                  }}
                  onFocus={(e) => {
                    e.currentTarget.style.boxShadow = `0 0 0 2px white, 0 0 0 4px ${primaryColor}`;
                  }}
                  onBlur={(e) => {
